 ...
 * @return std::vector<gen::Pose> vector of points on the path
 */
std::vector<gen::Pose> getData(const asset& path) {
    std::vector<gen::Pose> robotPath;
 ...
 */
int findClosest(gen::Pose pose, std::vector<gen::Pose> path) {
    int closestPoint;
 ...
 */
float circleIntersect(gen::Pose p1, gen::Pose p2, gen::Pose pose, float lookaheadDist) {
    // calculations
 ...
 */
gen::Pose lookaheadPoint(gen::Pose lastLookahead, gen::Pose pose, std::vector<gen::Pose> path, int closest,
                            float lookaheadDist) {
    ...
    for (int i = start; i < path.size() - 1; i++) {
        gen::Pose lastPathPose = path.at(i);
        gen::Pose currentPathPose = path.at(i + 1);

        float t = circleIntersect(lastPathPose, currentPathPose, pose, lookaheadDist);

        if (t != -1) {
            gen::Pose lookahead = lastPathPose.lerp(currentPathPose, t);
            lookahead.theta = i;
            return lookahead;
        }
    }
 ...
 */
float findLookaheadCurvature(gen::Pose pose, float heading, gen::Pose lookahead) {
    // calculate whether the robot is on the left or right side of the circle
    float side = gen::sgn(std::sin(heading) * (lookahead.x - pose.x) - std::cos(heading) * (lookahead.y - pose.y));
...
void gen::Chassis::follow(const asset& path, float lookahead, int timeout, bool forwards, bool async) {
    ...
    std::vector<gen::Pose> pathPoints = getData(path); // get list of path points
